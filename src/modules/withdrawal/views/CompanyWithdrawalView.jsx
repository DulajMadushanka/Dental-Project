...
import AddRiderView from "./EditCompanyWithdrawalView";
...
const WithdrawalRequestView = (props) => {
  const {currentUser, riderList, companyWithdrawalListData, openAlertBox, alertStatus, fetchPaginateWithdrawalRequests} = props;
  ...
  useEffect(() => {
    if (companyWithdrawalListData?.items?.length > 0) {
      const data = companyWithdrawalListData?.items.map((request) => {
        return {
  ...
  const onPaginateRequests = (type) => {
    const status = type;
    const preLastKey = companyWithdrawalListData.lastKeyList[companyWithdrawalListData.lastKeyList.length - 2];
    const nextLastKey = _.get(companyWithdrawalListData, 'nextLastKey', null);
  ...
  console.log("++++++++++++++++, withdrawalListData", companyWithdrawalListData)
  const isHasPrevItems = companyWithdrawalListData?.lastKeyList?.length > 0;
  const isHasNextItems = !_.isEmpty(companyWithdrawalListData?.nextLastKey);
                          ...
                          return (
                            <TableRow hover tabIndex={-1} key={row.code}>
                              {columns.map((column) => {
                                ...
                                const requestData = _.find(companyWithdrawalListData?.items, (request) => {
                                  return request.withdrawalId === row.id;
                                });
...
export default connect(state =>
    ({
      ...
      companyWithdrawalListData: state.withdrawal.get('companyWithdrawalListData'),
    }),
  ({
